...
// 递归遍历目录，收集符合要求的文件
const collectLakeboardFiles = (dir, files = []) => {
  const items = fs.readdirSync(dir);
  for (const item of items) {
    const fullPath = path.join(dir, item);
    const stat = fs.statSync(fullPath);
    if (stat.isDirectory()) {
      collectLakeboardFiles(fullPath, files);
    } else if (stat.isFile() && path.extname(item) === ".lakeboard") {
      files.push(fullPath);
    }
  }
  return files;
...
prompt([
  ...
  .then(async (answers) => {
    ...
    // 收集所有符合要求的文件
    const lakeboardFiles = collectLakeboardFiles(rootDir);
