...
import { convertLakeMindToContent } from "../script/convertLakeJsonToContentJson.js";
import { generateXMindFile } from "../script/generateXMindFile.js";
        ...
        .then(async (answers) => {
          ...
          const processFile = async (file) => {
            ...
            try {
              ...
              try {
                // 转换 .lakeboard 文件为 JSON
                const LakeJson = await convertLakeboardToJson(file);

                // 将 JSON 写入文件
                fs.writeFileSync(saveDir, JSON.stringify(LakeJson, null, 2));
                console.log(`文件 ${file} 已成功转换并保存为 ${saveDir}`);
              } catch (error) {
