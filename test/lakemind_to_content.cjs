...
const fileName = "lakemind.json";
const lakemindJsonPath = path.join(__dirname, "output", fileName);
// 读取 lakemind.json 文件
const lakemind = JSON.parse(fs.readFileSync(lakemindJsonPath, "utf8"));
...
const convertLakeMindToContent = async (lakemind) => {
  const convertTopic = (node, parentSummaries = []) => {
    // 如果node.html是富文本字符串，并且是a标签，提取href属性和text，topic的title为text，href为href
    ...
    const topic = {
      ...
      children: {
        // attached: node.children ? node.children.map(convertTopic) : [],
        attached: [],
        summary: [],
      },
      summaries: [],
    ...
    if (node.children) {
      node.children.forEach((child) => {
        if (child.abstract) {
          const summaryId = uuid.v4();
          console.log("====================================");
          console.log(parentSummaries);
          console.log("====================================");
          const newParentSummaries = parentSummaries
            ? [...parentSummaries]
            : []; // 确保始终为数组
    ...
    // // 处理子节点的summary，在父对象中添加summaries属性
    // if (topic.children.summary) {
    //   topic.summaries = topic.children.summary;
    //   const summaryId = uuid.v4();
  ...
  const rootTopic = {
    ...
    children: {
      attached: rootNode.children.map(convertTopic),
    },
  };
  ...
  const resultDir = path.join(__dirname, "output");
...
convertLakeMindToContent(lakemind);
